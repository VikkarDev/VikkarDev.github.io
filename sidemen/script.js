...
if (!localStorage.getItem('ytsidemen')) localStorage.setItem('ytsidemen', 0);
...
function start(gamemode = lastGm) {
    if (gamemode == 0) document.getElementById('high-score').innerHTML = `High Score: ${localStorage.getItem('ytsidemen')}`
    ...
    function onEnd() {
        ...
        timeouts.push(setTimeout(() => {
            if (win) { // If answer is correct or the same
                ...
                if (lastGm == 0 && (localStorage.getItem('ytsidemen') < score)) {
                    document.getElementById('high-score').innerHTML = `High Score: ${score}`
                    localStorage.setItem('ytsidemen', score)
