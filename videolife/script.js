...
if (!localStorage.getItem('ythvideo')) localStorage.setItem('ythvideo', 0);
...
function load() {
    document.getElementById('ytbtn').addEventListener('mouseover', () => {
        document.getElementById('mode-desc').innerHTML = "Pick the more popular option between two of VideoLife's 10+ YouTube videos"
...
function start(gamemode = lastGm) {
    if (gamemode == 0) document.getElementById('high-score').innerHTML = `High Score: ${localStorage.getItem('ythvideo')}`
    ...
    function onEnd() {
        ...
        timeouts.push(setTimeout(() => {
            if (win) { // If answer is correct or the same
                ...
                if (lastGm == 0 && (localStorage.getItem('ythvideo') < score)) {
                    document.getElementById('high-score').innerHTML = `High Score: ${score}`
                    localStorage.setItem('ythvideo', score)
